...
def test_directional_layout_blocks_disallowed_forward():
    layout = "...\n.g.\n.|."
    env = Warehouse(
        shelf_columns=1,
        column_height=3,
    ...
    env.step([Action.FORWARD.value])
    assert (agent.x, agent.y) == (1, 2)

    env.step([Action.LEFT.value])
    env.step([Action.FORWARD.value])
    assert (agent.x, agent.y) == (1, 1)


def test_cell_direction_constraints_are_applied():
    layout = "...\n.g.\n..."
    env = Warehouse(
        shelf_columns=1,
        column_height=3,
        ...
        reward_type=RewardType.GLOBAL,
        layout=layout,
        cell_direction_constraints=[{"x": 1, "y": 1, "dirs": "RIGHT"}],
    )
    env.reset(seed=2)
    assert env._is_direction_allowed(1, 1, Direction.RIGHT)
    assert not env._is_direction_allowed(1, 1, Direction.UP)
...
def test_lane_observation_and_available_directions_layer():
    layout = "...\n.-.\n.g."
    env = Warehouse(
        shelf_columns=1,
        column_height=3,
    ...
    agent.x, agent.y = 1, 1
    env._recalc_grid()
    obs = env._make_obs(agent)
    center = ((1 + 2 * env.sensor_range) ** 2) // 2
    assert obs["self"]["lane_up_map"][center] == 0
    assert obs["self"]["lane_down_map"][center] == 0
    assert obs["self"]["lane_left_map"][center] == 1
    assert obs["self"]["lane_right_map"][center] == 1
    ...
    env = Warehouse(
        ...
        layout="...\n.g.\n...",
        observation_type=ObservationType.IMAGE,
        image_observation_layers=[ImageLayer.AVAILABLE_DIRECTIONS],
        cell_direction_constraints=[{"x": 1, "y": 1, "dirs": ["UP", "RIGHT"]}],
    )
    env.reset(seed=4)
    layer = env.get_global_image(
        image_layers=[ImageLayer.AVAILABLE_DIRECTIONS], recompute=True
    )[0]
    assert layer[1, 1] == pytest.approx(9.0)
    assert layer[0, 0] == pytest.approx(10.0)


def test_obstacle_layout_blocks_forward_motion():
    layout = ".o.\n...\n.g."
    env = Warehouse(
        shelf_columns=1,
        column_height=3,
    ...
    agent.dir = Direction.RIGHT
    env._recalc_grid()
    _, _, _, _, info = env.step([Action.FORWARD.value])
    assert info["step_blocked_static"] == 1
    assert info["step_vertex_conflicts"] == 0


def test_dedicated_requests_assign_unique_requested_shelves():
    env = Warehouse(
        shelf_columns=3,
        column_height=8,
        shelf_rows=3,
    ...
    env.reset(seed=5)
    assert len(env.assigned_shelves) == 2
    assert len({shelf.id for shelf in env.assigned_shelves}) == 2
    assert env.request_queue == env.assigned_shelves
    assert env._is_requested_shelf(env.agents[0], env.assigned_shelves[0])
    assert not env._is_requested_shelf(env.agents[0], env.assigned_shelves[1])
