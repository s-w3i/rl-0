...
_SHELF_PADDING = 2
_SHELF_ID_COLOR = _WHITE
_AGENT_ID_COLOR = _WHITE
...
class Viewer(object):
    ...
    def _draw_shelfs(self, env):
        ...
        assigned_map = {}
        if getattr(env, "dedicated_requests", False) and getattr(env, "assigned_shelves", None):
            for agent_idx, shelf in enumerate(env.assigned_shelves, start=1):
                assigned_map[shelf] = agent_idx
        ...
        for shelf in env.shelfs:
            label_text = None
            if assigned_map:
                assigned_id = assigned_map.get(shelf)
                if assigned_id is None:
                    continue
                label_text = str(assigned_id)
            else:
                label_text = str(shelf.id)
            x, y = shelf.x, shelf.y
            y = self.rows - y - 1  # pyglet rendering is reversed
        ...
        for y in range(obstacles.shape[0]):
            for x in range(obstacles.shape[1]):
                if not obstacles[y, x]:
                    continue
                row = self.rows - y - 1  # pyglet rendering is reversed
                batch.add(
                    4,
                    gl.GL_QUADS,
                    None,
                    (
                        ...
                        (
                            ...
                            (self.grid_size + 1) * row + 1,  # TL - Y
                            (self.grid_size + 1) * (x + 1),  # TR - X
                            (self.grid_size + 1) * row + 1,  # TR - Y
                            (self.grid_size + 1) * (x + 1),  # BR - X
                            (self.grid_size + 1) * (row + 1),  # BR - Y
                            (self.grid_size + 1) * x + 1,  # BL - X
                            (self.grid_size + 1) * (row + 1),  # BL - Y
                        ),
                    ),
    ...
    def _draw_agents(self, env):
        ...
        for agent in env.agents:
            col, row = agent.x, agent.y
            row = self.rows - row - 1
            label_x = col * (self.grid_size + 1) + (1 / 2) * (self.grid_size + 1)
            label_y = (self.grid_size + 1) * row + (1 / 2) * (self.grid_size + 1)
            label = pyglet.text.Label(
                str(agent.id),
                font_name="Calibri",
                font_size=9,
